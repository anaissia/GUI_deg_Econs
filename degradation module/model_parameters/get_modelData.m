...
%% BATTERY GEOMETRY
data.thick1 = 75.50e-6;             % Thickness of anode [m]
data.thick2 = 25.00e-6;             % Thickness of separator [m]
data.thick3 = 74.00e-6;             % Thickness of cathode [m]
...
data.Rs1 = 2e-6;                 % Anode solid particles' radius [m]
data.Rs3 =  2e-6;                 % Cathode solid particles' radius [m]

% Volume fraction of active material [-]
data.eps1s = 0.51;                % anode
data.eps3s = 0.48;                % cathode
...
%for charge 
% data.x1_soc1 = 0.0115;%0.0068;              % at 0% soc in anode
% data.x1_soc0 = 0.83 %0.7560;              % at 100% soc in anode
% data.y3_soc1 = 0.95;              % at 0% soc in cathode
% data.y3_soc0 = 0.48 %0.4650;          	% at 100% soc in cathode
% for discharge
data.x1_soc1 = 0.008;%0.0068;              % at 0% soc in anode
data.x1_soc0 = 0.83; %0.7560;              % at 100% soc in anode
data.y3_soc1 = 0.95;              % at 0% soc in cathode
data.y3_soc0 = 0.48; %0.4650;          	% at 100% soc in cathode
% Diffusion coefficient of Li in active material [m2.s-1]
data.Ds1_ref = 3.8e-14;     % Anode diffusion coeff @ ref temperature [m2.s-1]
data.Ds3_ref = 1.0e-13;     % Cathode diffusion coeff @ ref temperature [m2.s-1]
...
data.k1_ref = 1e-6;%1.764e-11;
data.k3_ref = 1e-6;%6.667e-11;
