...
from read_volt import read_V
import sys

def econs(path,capacity,unit,start_passenger,eff_t,eff_convert,reg_brak_eff,gear_ratio,radius,M,A, rho_air,Cdrag,aux_load, T_max, P_max):
    """
...
#            P_instantaneous.append((P_instantaneous_w[x]/(eff[x]*eff_convert))+aux_load)# get the power consumed by the motor with regenerative braking
    ...
    voltage=[x*168 for x in voltage] # this is for Lajujen
    curr=[P_instantaneous[i]/voltage[i] for i in range(0,len(voltage))]
    branches_series=6 #lajujen
    curr=[i/branches_series for i in curr] #current equals in each branch
    cap_cell=40 #Ah
    c_rate=[x/cap_cell for x in curr]
    f=open(path+'/crate.csv','w')
    dimension=len(c_rate)
    for i,j in zip(time_diff[0:dimension],c_rate[0:dimension]):
        f.write(str(i)+","+str(j)+"\n")
    f.close()
    
    # there are four branches in a battery, so divide the current by 4
    print("its over!")
...
#econs('Brampton_23_East_Heavy Duty',150,unit,start_passenger,eff_t,eff_convert,reg_brak_eff,gear_ratio,radius,M,A, rho_air,Cdrag,aux_load, T_max, P_max)